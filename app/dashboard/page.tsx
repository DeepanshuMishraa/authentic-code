...
export default function Dash() {
  ...
  const filteredRepos = useMemo(() => {
    if (!query.data || !searchQuery) return query.data;

    const normalizedSearch = searchQuery.toLowerCase();
    return query.data.filter(repo =>
  ...
  if (query.isLoading) {
    return (
      <motion.div
        ...
        className="min-h-[90vh] flex flex-col items-center justify-center p-4"
      >
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {filteredRepos?.length === 0 ? (
              <motion.div
                ...
                layoutId="repoContainer"
              >
                <div className="text-center space-y-4">
                  <svg className="w-16 h-16 text-muted-foreground/50 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
